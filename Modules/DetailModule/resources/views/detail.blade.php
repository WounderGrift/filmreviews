...
@section('content')
    ...
    @if (!isset($film) || !isset($film->detail))
        <div class="blog">
        ...
        <div class="popup-subscribe popup disabled">
            <div class="popup-content subscribe disabled">
                ...
                <form id="subscribe-form" method="POST" onsubmit="return false;">
                    <input id="subscribe-email" class="regist-login" type="email" name="subscribe"
        ...
        <div class="popup-send-error popup disabled">
            <div class="popup-content send-error disabled">
                ...
                <form id="send-error-form" method="POST" onsubmit="return false;">
                    ...
                    <textarea id="send-error-text" class="regist-login" type="text" name="error-report"
                              placeholder="Описание ошибки" autocomplete="off"></textarea>
            ...
            <div class="container" data-film-id="{{ base64_encode($film->id) }}">
                <h2 class="title">
                    @if (Auth::check() && Auth::user()->checkOwnerOrAdmin())
                        ...
                        @endif
                        {{ $film->name }}
                        @if (!$film->trashed() && ($film->status === $film::STATUS_UNPUBLISHED && Auth::user()->checkAdmin() || Auth::user()->checkOwner()))
                ...
                <div class="blog-left">
                    <div class="blog-info">
                        <div class="blog-info-text">
                                    ...
                                    <div id="media-normal" class="summary">
                                        <ul class="requirement-list">
                                            <li class="requirement-item">
                                                ...
                                                <span
                                                    class="requirement-value">{!! \App\Http\Helpers\DetailHelper::getCategoriesForView($detail) !!}</span>
                                            </li>
                                            <li class="requirement-item">
                                                <span class="requirement-label">Дата выхода:</span>
                                                <span
                                                    class="requirement-value">{{ \App\Http\Helpers\DateHelper::dateFormatterForDetail($film->date_release, config('app.timezone')) }}</span>
                                            </li>

                                            @if (isset($film->series) && ($showSeries || ( Auth::check() && Auth::user()->checkOwnerOrAdmin())))
                                                <li class="requirement-item">
                                                    <span class="requirement-label">Серия:</span>
                                                    <a class="requirement-value series-link"
                                                       href="{{ route('series.indexSeries', ['uri' => $film->series->uri]) }}">{{ $film->series->name }}</a>
                                                </li>
                                            @endif
                                            ...
                                            @foreach ($info->summary as $key => $value)
                                                <li class="requirement-item">
                                                    ...
                                                    <span
                                                        class="requirement-value">{!! \App\Http\Helpers\DetailHelper::getSeparatedSummary($value) !!}</span>
                                                </li>
                                            @endforeach
                                            @if (isset($isRecommended) && !empty($recommended))
                                                <a class="recommended btn btn-info"
                                                   href="{{ route('recommended.index', ['ids' => $recommended]) }}">
                                                    {{ $buttonTitle }}
                                                </a>
                                            @elseif (isset($isRecommended))
                                                <a class="recommended btn btn-info" href="{{ route('main.index') }}">Вернутся
                                                    к рекомендациям</a>
                                            @endif
                                        </ul>
                                    </div>
                            ...
                            @if ($minSysCount > 0 || $maxSysCount > 0)
                                <h4>Какие-то системные требования</h4>
                            @endif
                            ...
                            @if ($basementBanner->isNotEmpty())
                                ...
                                <x-bannersmodule.banner-detail
                                    :banners="$basementBanner"></x-bannersmodule.banner-detail>
                            @endif

                            @if (!$film->is_waiting)
                                @foreach($detail?->files->sortBy('version') as $file)
                                    @if (!$film->is_sponsor && !$file->is_link && !Storage::disk('public')->exists($file->path))
                                        @continue
                                    @endif
                                                ...
                                                <div class="spoiler-header"
                                                    ...
                                                    <div style="display: flex; align-items: flex-start;">
                                                        <h2 class="download-title">
                                                            {{ $file?->repacks?->label && $file?->size ? " | " : " " }}
                                                            {{ $file?->size ? "Размер: {$file->size}" : "" }}
                                                            {{ $file?->size && $file?->version && $file?->version != "v0.0" ? " | " : " " }}
                                                            {{ $file?->version && $file?->version != "v0.0" ? "{$file->version}" : "" }}
                                                        </h2>
                            ...
                            <div class="outline">
                                <div class="text-center">
                                    <a href="{{ route('faq') }}" target="_blank">FAQ по проблемам с установкой и
                                        запуском репаков</a>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="comment-icons"
                            ...
                            <ul>
                                <li>
                                    <i class="far fa-calendar"></i>{{ \App\Http\Helpers\DateHelper::dateFormatterForDetail($film->created_at, config('app.timezone')) }}
                                </li>
                                ...
                                @if (Auth::check())
                                    @if (Auth::user()->is_verify)
                                        <li>
                                            <label class="heart-detail wishlist-action">
                                                <input type="checkbox" class="checkbox-detail"
                                                    {{ Auth::user()->wishlist->contains('id', $film->id) ? 'checked' : '' }}>
                                        ...
                                        <i class="far fa-heart"></i>
                                            ...
                                            <span class="wishlist favorite-count">{{ count($film->wishlist) }}</span>
                                        </li>
                                    @else
                                        <li><i class="far fa-heart"></i>{{ count($film->wishlist) }}</li>
                                    @endif
                                    <li>
                                        <label class="heart-detail like-action film">
                                            <input type="checkbox" class="checkbox-detail"
                                                {{ Auth::user()->likes->where('film_id', $film->id)->whereNull('comment_id')->isNotEmpty() ? 'checked' : '' }}>
                                            <span class="detail-heart-icon">
                                        ...
                                        <span class="like favorite-count">{{ count($film->likes) }}</span>
                                    </li>
                                @else
                                    <li>
                                ...
                                @if (Auth::check())
                                    @if (!Auth::user()->newsletters->contains($film->id))
                                ...
                                @endif
                                <li class="newsletter_count">{{ $film->newsletters->count() }}</li>
                            </ul>
                                            ...
                                            @if (Auth::check())
                                                <li>
                                                    <label class="heart-detail like-action comment">
                                                        <input type="checkbox" class="checkbox-detail"
                                                               data-comment-id="{{ base64_encode($comment->id) }}"
                                                            {{ Auth::user()->likes->where('film_id', $film->id)->where('comment_id', $comment->id)->isNotEmpty() ? 'checked' : '' }}>
                                                        <span class="detail-heart-icon">
                                                        <i class="far fa-star"></i>
                                                    </span>
                                                        <span class="detail-heart-icon-filled">
                                                    ...
                                                    <span
                                                        class="like favorite-count">{{ count($comment->likes) }}</span>
                                                </li>

                                                <li class="reply" data-comment-id="{{ base64_encode($comment->id) }}">
                                                    Ответить
                                                </li>
                                                @if ($comment->user->id === Auth::check() && (Auth::user()->id || Auth::user()->checkOwnerOrAdmin()))
                                                    <li class="remove"
                                                        data-comment-id="{{ base64_encode($comment->id) }}"
                                                        data-hard="false">Удалить
                                                    </li>
                                                @endif
                                                @if (Auth::check() && Auth::user()->checkOwnerOrAdmin())
                                                    <li class="remove"
                                                        data-comment-id="{{ base64_encode($comment->id) }}"
                                                        data-hard="true">Удалить Жестко
                                                    </li>
                                                @endif
                                            @endif
                                        </ul>
                    ...
                    @if (Auth::check() && Auth::user()->is_verify && !Auth::user()->is_banned)
                        ...
                        <div class="comment-form" data-film-uri="{{ $film->uri }}">
                            ...
                            <form id="response-form" method="POST">
                                ...
                                <textarea id="comment-textarea" type="text" name="comment"
    ...
    <script type="module"
            src="{{ asset('Modules/DetailModule/resources/assets/js/detail.js') }}?version={{ config('app.version') }}"></script>
    @if(File::exists(base_path('Modules/MainModule')))
        <script type="module"
                src="{{ asset('Modules/MainModule/resources/assets/js/wishlistOnGrid.js') }}?version={{ config('app.version') }}"></script>
    @endif
